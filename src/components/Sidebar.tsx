...
export default function Sidebar({ role }: { role: string }) {
  ...
  return (
    <aside className="hidden md:flex flex-col w-72 bg-white border-r border-slate-200 min-h-screen shrink-0">
      {/* Logo */}
      <div className="flex items-center gap-3 px-6 py-6 border-b border-slate-100">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">Daily Report System</p>
        </div>
      </div>

      {/* Navigation */}
      <nav className="flex-1 p-4 space-y-1">
        <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider px-4 pt-2 pb-2">
        ...
        {(role === "MANAGER" ? managerItems : employeeItems).map((item) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <ChevronRight className="w-4 h-4 ml-auto text-blue-400 shrink-0" />
              )}
            </Link>
          );
        })}
